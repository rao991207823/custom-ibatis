...
import com.gupao.raos.ibatis.IbatisTest;
import com.gupao.raos.ibatis.annotation.Entity;
import com.gupao.raos.ibatis.annotation.Select;
import com.gupao.raos.ibatis.binding.MapperRegistry;
import com.gupao.raos.ibatis.executor.CachingExecutor;
import com.gupao.raos.ibatis.executor.Executor;
import com.gupao.raos.ibatis.executor.SimpleExecutor;
import com.gupao.raos.ibatis.plugin.Interceptor;
import com.gupao.raos.ibatis.plugin.InterceptorChain;
...
public class Configuration {
    ...
    private void scanPackage(String mapperPath) {
        String classPath = IbatisTest.class.getResource("/").getPath();
        ...
        for (String className : classPaths) {
            className = className.replace(classPath.replace("/", "\\")
                    .replaceFirst("\\\\", ""), "")
                    //截取全类名
                    .substring(className.indexOf(mapperPath), className.length())
                    .replace("\\", ".")
                    .replace(".class", "");
            Class<?> clazz = null;
            try {
